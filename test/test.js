...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof incrnanmmpe, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an accumulator function', function test( t ) {
	t.equal( typeof incrnanmmpe( 3 ), 'function', 'returns a function' );
	t.end();
});
...
tape( 'if data has yet to be provided, the accumulator function returns `null`', function test( t ) {
	var acc = incrnanmmpe( 3 );
	t.equal( acc(), null, 'returns null' );
	t.end();
});

tape( 'if provided `NaN`, the accumulated value is current non-NaN mean percentage error', function test( t ) {
	var expected;
	var data;
	var acc;
	var v;
